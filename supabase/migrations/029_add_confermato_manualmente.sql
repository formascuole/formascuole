-- Migration 029: Add confermato_manualmente to corsi
-- Tracks whether a calendar was manually confirmed by admin/formatore
-- rather than via the school's email confirmation link.

ALTER TABLE corsi ADD COLUMN IF NOT EXISTS confermato_manualmente boolean DEFAULT false;

-- Recreate corsi_con_ore to include the new column (c.* picks it up automatically,
-- but we drop and recreate to also refresh progetti_con_stats which CASCADE-depends on it).

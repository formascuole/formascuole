-- Recreate corsi_con_ore so it includes all columns added after the view was
-- originally created (edizione, note, referente_corso_ruolo, fattura_ricevuta,
-- fattura_ricevuta_at, etc.).  PostgreSQL views freeze their column list at
-- CREATE time, so c.* does NOT pick up new table columns automatically.
...
-- Recreate progetti_con_stats (was CASCADE-dropped above)

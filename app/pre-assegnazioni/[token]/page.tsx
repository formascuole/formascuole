import { PreAssegnazioniClient } from './PreAssegnazioniClient'

const APP_URL = process.env.NEXT_PUBLIC_APP_URL || 'https://formascuole.vercel.app'

export default async function PreAssegnazioniPage({ params }: { params: Promise<{ token: string }> }) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-xl p-8 max-w-md w-full text-center shadow-sm border border-gray-100">
          ...
          <p className="text-sm text-gray-500">Impossibile caricare i dati. Riprova più tardi.</p>
        </div>
      </div>
    )
  }
